...
def test_saver_compress():
    x = lego.reader.read_vocab_file(Path("tests/python_src.lego/test_vocab_files/regular_list.txt"))
    lego.saver.save_vocab_dump(Path("tests/python_src.lego/test_vocab_files/testdump/regular_list.testdump.lz4"), x, compress=True)
    y = lego.reader.read_vocab_dump(Path("tests/python_src.lego/test_vocab_files/testdump/regular_list.testdump.lz4"))
    assert x == y
...
def test_saver_compress_nolz4extension():
    x = lego.reader.read_vocab_file(Path("tests/python_src.lego/test_vocab_files/regular_list.txt"))
    with pytest.warns(MisleadingFilenameWarning, match=re.escape("The file 'tests/python_src.lego/test_vocab_files/testdump/regular_list.compressedtestdump' is being compressed, but the '.lz4' extension is not present and is being added.")):
        lego.saver.save_vocab_dump(Path("tests/python_src.lego/test_vocab_files/testdump/regular_list.compressedtestdump"), x, compress=True)
    y = lego.reader.read_vocab_dump(Path("tests/python_src.lego/test_vocab_files/testdump/regular_list.compressedtestdump.lz4"))
    assert x == y
