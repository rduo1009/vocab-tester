...
following traits:
        ...
        The passed string is encrypted with a simple XOR cipher at compile-time
to prevent it being viewable in the binary image
        - Global lifetime
        ...
        This means that you can pass it directly into functions that would
normally take a char* or a const char*
...
// Workaround for __LINE__ not being constexpr when /ZI (Edit and Continue) is
// enabled in Visual Studio See:
...
// libstdc++ has std::remove_cvref_t<T> since C++20, but because not every user
// will be able or willing to link to the STL, we prefer to do this
// functionality ourselves here.
template <typename T> struct remove_const_ref {
  using type = T;
};

template <typename T> struct remove_const_ref<T &> {
  using type = T;
};
...
template <typename T> using char_type = typename remove_const_ref<T>::type;
...
#define AY_OBFUSCATE_KEY(data, key)                                            \
  []() -> ay::obfuscated_data<sizeof(data) / sizeof(data[0]), key,             \
                              ay::char_type<decltype(*data)>> & {              \
    static_assert(sizeof(decltype(key)) == sizeof(ay::key_type),               \
                  "key must be a 64 bit unsigned integer");                    \
    ...
    thread_local auto obfuscated_data =                                        \
        ay::obfuscated_data<n, key, char_type>(obfuscator);                    \
