...
class LearningVerb(_Word):
    ...
    def __init__(
        ...
        *,
        ...
        self.endings = LearningVerb._same_regular_endings(self.present, self.infinitive, self.perfect)

        self.endings.update(
            LearningVerb._similar_regular_endings(self.conjugation, self.present, self.infinitive, self.perfect)
        )
        self.endings.update(
            LearningVerb._static_regular_endings(self.conjugation, self.present, self.infinitive, self.perfect)
        )

        if self.ppp:
            self.endings.update(LearningVerb._participle_endings(self.ppp, self.infinitive))
    ...
    def _same_regular_endings(present: str, infinitive: str, perfect: str) -> Endings:
        ...
        VERB_SAME_ENDINGS: dict[str, tuple[str, tuple[str, ...]]] = ending_tables.VERB_SAME_ENDINGS(
            infinitive=infinitive, per_stem=per_stem
        )
        return {
            f"V{tense}{voice}{mood}{number}{person}": VERB_SAME_ENDINGS[f"{tense}{voice}{mood}"][0]
    ...
    def _similar_regular_endings(
        ...
        VERB_SIMILAR_ENDINGS: dict[str, tuple[str, tuple[str, ...], tuple[Union[str, None], ...]]] = (
            ending_tables.VERB_SIMILAR_ENDINGS(inf_stem=inf_stem)
        )

        return {
            f"V{tense}{voice}{mood}{number}{person}": VERB_SIMILAR_ENDINGS[f"{tense}{voice}{mood}"][0]
...
class Noun(_Word):
    ...
    def __init__(
        ...
        *,
        ...
        DECLENSION_MAP: dict[str, tuple[Literal[1, 2, 3, 4, 5], int, Optional[bool]]] = {
            "ei": (5, -2, None),
            "ae": (1, -2, None),
            "i": (2, -1, None),
            "is": (3, -2, None),
        ...
        self.endings.update(Noun._static_regular_endings(self.nominative, self.genitive, self._stem, self.declension))
        ...
        if self.plurale_tantum:
            self.endings = {k: v for k, v in self.endings.items() if not k.endswith("sg")}
        ...
        return {
            ...
            if (
                suffix := ending_tables.NOUN_ENDINGS[
                    index_from_value(CASE_SHORTHAND, case) + ((declension - 1) * 12) + (6 if number == "pl" else 0)
                ]
            )
        }

    @staticmethod
    def _static_regular_endings(nom: str, gen: str, stem: str, declension: Literal[1, 2, 3, 4, 5]) -> Endings:
        match declension:
            case 1 | 3 | 4 | 5:
                return {
                    "Nnomsg": nom,  # puella
...
class Adjective(_Word):
        ...
        *principal_parts : str
        ...
        match self.declension:
            case "212":
                ...
                self.endings.update(
                    Adjective._endings_from_table(ending_tables.ADJECTIVE1_ENDINGS, self._pos_stem, "pos", "f")
                )
                self.endings.update(
                    Adjective._endings_from_table(ending_tables.ADJECTIVE2N_ENDINGS, self._pos_stem, "pos", "n")
                )
                self.endings.update(
                    {
            ...
            case "3":
                ...
                self.endings.update(
                    Adjective._endings_from_table(ending_tables.ADJECTIVE3_ENDINGS, self._pos_stem, "pos", "m")
                )
                self.endings.update(
                    Adjective._endings_from_table(ending_tables.ADJECTIVE3_ENDINGS, self._pos_stem, "pos", "f")
                )
                self.endings.update(
                    Adjective._endings_from_table(ending_tables.ADJECTIVE3N_ENDINGS, self._pos_stem, "pos", "n"),
                )
            case _:
                raise InvalidInputError(f"Declension {self.declension} not recognised")

        self.endings.update(
            Adjective._endings_from_table(ending_tables.ADJECTIVE3C_ENDINGS, self._cmp_stem, "cmp", "m")
        )
        self.endings.update(
            Adjective._endings_from_table(ending_tables.ADJECTIVE3C_ENDINGS, self._cmp_stem, "cmp", "f")
        )
        self.endings.update(
            Adjective._endings_from_table(ending_tables.ADJECTIVE3CN_ENDINGS, self._cmp_stem, "cmp", "n")
        )
        ...
        self.endings.update(Adjective._endings_from_table(ending_tables.ADJECTIVE2_ENDINGS, self._spr_stem, "spr", "m"))
        self.endings.update(Adjective._endings_from_table(ending_tables.ADJECTIVE1_ENDINGS, self._spr_stem, "spr", "f"))
        self.endings.update(
            Adjective._endings_from_table(ending_tables.ADJECTIVE2N_ENDINGS, self._spr_stem, "spr", "n")
        )
        self.endings.update(
            {
    ...
    def _comparative_stem(self) -> None:
        ...
        if self._mascnom in edge_cases.IRREGULAR_COMPARATIVES:
            ...
            self._irregular_posadv: str = edge_cases.IRREGULAR_COMPARATIVES[self._mascnom][2]
            self._irregular_cmpadv: str = edge_cases.IRREGULAR_COMPARATIVES[self._mascnom][3]
            self._irregular_spradv: str = edge_cases.IRREGULAR_COMPARATIVES[self._mascnom][4]
    ...
    def _endings_from_table(
        ...
        return {
            ...
            for count, case in enumerate(CASE_SHORTHAND.values())
            for number in NUMBER_SHORTHAND.values()
            if (suffix := endings_table[index_from_value(CASE_SHORTHAND, case) + (6 if number == "pl" else 0)])
        }
